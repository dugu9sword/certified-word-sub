...
def parse_args():
    ...
    parser.add_argument(
        'model', choices=['bow', 'cnn', 'lstm', 'decomp-attn', 'lstm-final-state'])
    parser.add_argument(
        'out_dir', help='Directory to store and load output', default='out')
    # Model
    ...
    parser.add_argument(
        '--pool', choices=['max', 'mean', 'attn'], default='max')
    parser.add_argument('--num-layers', type=int, default=3,
                        help='Num layers for SNLI baseline BOW model')
    parser.add_argument('--no-wordvec-layer', action='store_true',
                        help="Don't apply linear transform to word vectors")
    parser.add_argument('--early-ibp', action='store_true',
                        help="Do to_interval_bounded directly on base word vectors")
    parser.add_argument('--no-relu-wordvec', action='store_true',
                        help="Don't do ReLU after word vector transform")
    parser.add_argument('--unfreeze-wordvec',
                        action='store_true', help="Don't freeze word vectors")
    parser.add_argument(
    ...
    parser.add_argument('--use-lm', action='store_true',
                        help='Use LM scores to define attack surface')
    # Training
    ...
    parser.add_argument('--data-cache-dir', '-D',
                        help='Where to load cached dataset and glove')
    parser.add_argument('--neighbor-file', type=str,
                        default=data_util.NEIGHBOR_FILE)
    parser.add_argument('--glove-dir', type=str, default=vocabulary.GLOVE_DIR)
    parser.add_argument('--imdb-dir', type=str,
                        default=text_classification.IMDB_DIR)
    parser.add_argument('--agnews-dir', type=str,
                        default=text_classification.AGNEWS_DIR)
    ...
    parser.add_argument('--use-agnews-data', action='store_true')
...
def main():
    ...
    train_data, dev_data, word_mat, attack_surface = task_class.load_datasets(
        device, OPTS)
