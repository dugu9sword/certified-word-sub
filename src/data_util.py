...
class ProcessedDataset(Dataset):
    ...
    def get_loader(self, batch_size):
        batch_sampler = PooledBatchSampler(
            self, batch_size, sort_key=self.example_len)
...
def multi_dim_padded_cat(tensors, dim, padding_value=0):
    ...
    """
    if dim == 0:
        original_ordering = dim_first_ordering = list(
            range(len(tensors[0].shape)))
    else:
    ...
    for tensor in tensors:
        ...
        out_tensor[cur_idx:cur_idx +
                   out_shape[0], ...] = F.pad(tensor.permute(*dim_first_ordering), pad)
        cur_idx += out_shape[0]
    if dim != 0:
...
class PooledBatchSampler(BatchSampler):
    ...
    def __iter__(self):
        ...
        5- Yields each batch
        """
        idx_chunks = torch.split(torch.randperm(
            len(self.dataset_lens)), self.batch_size * 100)
        for idx_chunk in idx_chunks:
            sorted_chunk = torch.tensor(
                sorted(idx_chunk.tolist(), key=lambda idx: self.dataset_lens[idx]))
            chunk_batches = [chunk.tolist() for chunk in torch.split(
                sorted_chunk, self.batch_size)]
