...
def test_bmm():
    m1 = torch.tensor([[-1, 2], [3, 2], [-3, 1]],
                      dtype=torch.float).view(1, 3, 2)
    m2 = torch.tensor([[4, 5], [-4, -5]], dtype=torch.float).view(1, 2, 2)
    z = ibp.bmm(ibp.IntervalBoundedTensor(m1, m1, m1),
                ibp.IntervalBoundedTensor(m2, m2, m2))
    m1_bounded = ibp.IntervalBoundedTensor(
        m1, m1 - torch.tensor(0.1), m1 + torch.tensor(0.1))
    m2_bounded = ibp.IntervalBoundedTensor(
        m2, m2 - torch.tensor(0.1), m2 + torch.tensor(0.1))
