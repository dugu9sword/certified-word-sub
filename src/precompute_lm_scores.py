...
def main():
    ...
    if OPTS.shard is not None:
        ...
        data = data[OPTS.shard *
                    OPTS.shard_size:(OPTS.shard + 1) * OPTS.shard_size]
        ...
        for sent_idx, example in enumerate(tqdm(data)):
            ...
            for i, w in enumerate(words):
                if w in neighbors:
                    ...
                    prefix = [x for x in words[start:i]
                              if x in query_handler.word_to_idx]
                    suffix = [x for x in words[i+1:end]
                              if x in query_handler.word_to_idx]
                    queries = []
                    in_vocab_options = []
                    for opt in options:
