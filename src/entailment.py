  ...
  """
    ...
    try:
        if opts.adv_only:
            ...
            with open(os.path.join(opts.data_cache_dir, 'train_data.pkl'),
                      'rb') as infile:
            ...
            with open(
                    os.path.join(opts.data_cache_dir,
                                 'train_attack_surface.pkl'), 'rb') as infile:
                train_attack_surface = pickle.load(infile)
        with open(os.path.join(opts.data_cache_dir, 'dev_data.pkl'),
                  'rb') as infile:
            dev_data = pickle.load(infile)
            if not isinstance(dev_data, data_class):
                raise Exception("Cached dataset of wrong class: {}".format(
                    type(train_data)))
        with open(os.path.join(opts.data_cache_dir, 'word_mat.pkl'),
                  'rb') as infile:
            word_mat = pickle.load(infile)
        with open(os.path.join(opts.data_cache_dir, 'dev_attack_surface.pkl'),
                  'rb') as infile:
            dev_attack_surface = pickle.load(infile)
        print("Loaded data from {}.".format(opts.data_cache_dir))
    except Exception as ex:
        print('Couldn\'t load data from cache: {}, reading from raw files'.
              format(ex))
        if opts.adv_only:
            train_data = None
            train_attack_surface = None
        else:
            train_attack_surface = attacks.WordSubstitutionAttackSurface.from_file(
                opts.neighbor_file)
        if opts.use_lm:
            dev_attack_surface = attacks.LMConstrainedAttackSurface.from_files(
                opts.neighbor_file, opts.snli_lm_file)
        else:
            dev_attack_surface = attacks.WordSubstitutionAttackSurface.from_file(
                opts.neighbor_file)
        raw_data = data_class.get_raw_data(opts)
        word_set = raw_data.get_word_set(train_attack_surface,
                                         dev_attack_surface=dev_attack_surface)
        ...
        if not opts.adv_only:
            train_data = data_class.from_raw_data(
                raw_data.train_data,
                vocab,
                attack_surface=train_attack_surface,
        ...
        dev_data = data_class.from_raw_data(
            raw_data.dev_data,
            vocab,
            attack_surface=dev_attack_surface,
            downsample_to=opts.downsample_to,
            prepend_null=opts.prepend_null,
            use_tqdm=True,
            downsample_shard=opts.downsample_shard)
        if opts.data_cache_dir:
            ...
            with open(
                    os.path.join(opts.data_cache_dir,
                                 'dev_attack_surface.pkl'), 'wb') as outfile:
...
def compute_is_correct(model_output, gold_labels):
    if isinstance(model_output, ibp.IntervalBoundedTensor):
        ...
        worst_case_pred = (gold_labels * model_output.lb +
                           (1 - gold_labels) * model_output.ub).argmax(dim=1)
  ...
  """
    ...
    def from_raw_data(cls,
        ...
        for inpt, y in iteration:
            try:
                examples.append(
                    cls.process_example(inpt,
                                        y,
                                        vocab,
                                        attack_surface,
                                        prepend_null=prepend_null))
        ...
        for split in split_names:
            ...
            with open(fn) as f:
                for line in tqdm(f, total=totals[split]):
                    ...
                    prem, hypo, gold_label = example['sentence1'], example[
                        'sentence2'], example['gold_label']
  ...
  """
    def run(self, model, dataset, device, opts=None):
        ...
        for x, y in dataset.raw_data:
            ...
            all_raw = [(prem, (' '.join(hypo_new)), y)
                       for hypo_new in itertools.product(*choices)]
            ...
            cur_adv_exs = [
                all_raw[i][0] for i, p in enumerate(model_correct) if p.item()
            ]
            print(cur_adv_exs)
    ...
    def run(self, model, dataset, device, opts=None):
        ...
        for x, y in tqdm(dataset.raw_data):
            ...
            orig_pred, orig_gold = model.query(EntailmentDataset.from_raw_data(
                [(x, y)], dataset.vocab, attack_surface=self.attack_surface),
                                               device,
                                               return_bounds=True)
            model_correct, model_cert_correct = compute_is_correct(
                orig_pred, orig_gold)
                ...
                for epoch in range(self.num_epochs):
                    ...
                    for i in word_idxs:
                        ...
                        if margins[best_idx] < self.margin_goal:
                            ...
                            print(
                                'ADVERSARY SUCCESS on ("%s", %s): Found "%s" with margin %.2f'
...
class GeneticAdversary(Adversary):
    ...
    def perturb(self,
        ...
        if all(len(c) == 1 for c in choices):
            value_margin, _ = get_margins(*(model.query(
                EntailmentDataset.from_raw_data([((prem, ' '.join(hypo)), y)],
                                                vocab,
                                                prepend_null=prepend_null),
                device)))
        ...
        for w_new in choices[idx]:
            cur_raw = [((prem,
                         ' '.join(hypo[:idx] + [w_new] + hypo[idx + 1:])), y)]
            cur_dataset = EntailmentDataset.from_raw_data(
                cur_raw, vocab, prepend_null=prepend_null)
            model_output, gold_labels = model.query(cur_dataset, device)
            value_margins, worst_case_margins = get_margins(
                model_output, gold_labels)
            if best_replacement is None or value_margins[0].item(
            ) < worst_margin:
    ...
    def run(self, model, dataset, device, opts=None):
        ...
        for x, y in tqdm(dataset.raw_data):
            ...
            choices = [[w] + cur_swaps
                       for w, cur_swaps in zip(hypo_words, swaps)]
            found = False
            population = [
                self.perturb(prem,
                             hypo_words,
            ...
            for g in range(self.num_iters):
                ...
                for i in range(1, self.pop_size):
                    parent1 = population[np.random.choice(range(
                        len(population)),
                                                          p=sample_probs)][0]
                    parent2 = population[np.random.choice(range(
                        len(population)),
                                                          p=sample_probs)][0]
                    child = [
                        random.sample([w1, w2], 1)[0]
                        for (w1, w2) in zip(parent1, parent2)
                    ]
                    child_mut, new_margin = self.perturb(
                        prem,
                        child,
                        choices,
                        model,
